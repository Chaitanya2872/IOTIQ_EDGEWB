...
const CompactHeatmap: React.FC<{
  ...
  const [heatmapData, setHeatmapData] = useState<HeatmapDataItem[]>([]);
  const unit = item.unit || item.unitOfMeasurement || 'units';
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: COLORS.gray, margin: '4px 0 0 0' }}>
              Daily consumption pattern • {heatmapData.length} records • Unit: {unit}
            </p>
          </div>
          <button
            onClick={onClose}
                ...
                <div key={monthKey}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '3px' }}>
                    ...
                    {monthData.map(data => (
                      <div
                        key={data.date}
                        title={`${data.date}: ${data.consumption} ${unit}`}
                        style={{
                          aspectRatio: '1',
                          backgroundColor: getColor(data.intensity),
                          borderRadius: '4px',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '11px', color: COLORS.gray }}>
                        {data.avgDaily.toFixed(1)} {data.item.unit || data.item.unitOfMeasurement || 'units'}/day
                      </div>
                    </div>
                  </div>
...
const SmartInsightsFeed: React.FC<{
  ...
  const insights = useMemo(() => {
    ...
    if (criticalItems.length > 0) {
      ...
      const reorderQty = Math.ceil(item.avgDaily * 30);
      const fillPercentage = Math.round((item.currentStock / (item.item.maxStockLevel || item.currentStock + 100)) * 100);
      const unit = item.item.unit || item.item.unitOfMeasurement || 'units';
      ...
      insights.push({
        ...
        mainValue: Math.floor(item.coverage),
        mainLabel: 'Days Left',
        secondaryValue: `${item.currentStock} ${unit}`,
        secondaryLabel: 'Current Stock',
        fill: fillPercentage,
        recommendation: `Order ${reorderQty} ${unit} to maintain 30-day supply`,
        color: '#ef4444',
        bgColor: '#fef2f2'
      });
    }
    ...
    if (spikes.length > 0) {
      ...
      insights.push({
        ...
        mainValue: `+${item.trend.toFixed(0)}%`,
        mainLabel: 'Increase',
        secondaryValue: `${item.lastWeekAvg.toFixed(1)} ${unit}/day`,
    ...
    if (categoryArray.length >= 1) {
      ...
      insights.push({
        ...
        item: topCategory.name,
        category: `${topCategory.items} items`,
        mainValue: `${topCategory.avgDaily.toFixed(1)} units/day`,
        mainLabel: 'Avg Consumption',
        secondaryValue: `${topCategory.total.toFixed(0)} units`,
        secondaryLabel: 'Weekly Total',
        fill: categoryPercentage,
        recommendation: `${categoryPercentage}% of total inventory activity`,
        color: '#06b6d4',
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {item.avgDailyConsumption && (
                          <div style={{ 
                            ...
                            {Number(item.avgDailyConsumption).toFixed(1)} {item.unit || item.unitOfMeasurement || 'units'}/day
                          </div>
                        )}
                      </div>
                    </label>
...
const ConsumptionChart: React.FC<{
  ...
  const [loading, setLoading] = useState(false);
  const [chartData, setChartData] = useState<any[]>([]);
  const [displayUnit, setDisplayUnit] = useState<string>('units');
  ...
  useEffect(() => {
    ...
    try {
      ...
      const mostCommonUnit = Array.from(unitCounts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] || 'units';
      setDisplayUnit(mostCommonUnit);
      ...
      <div style={{ marginBottom: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '12px', color: COLORS.gray, margin: 0 }}>
              {selectedItemsText ? (
                <>Daily usage for <span style={{ fontWeight: '600', color: COLORS.primary }}>{selectedItemsText}</span> • Unit: {displayUnit}</>
              ) : (
                <>Daily usage patterns over time • Unit: {displayUnit}</>
              )}
            </p>
          </div>
          <div style={{ display: 'flex', gap: '10px' }}>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '10px', color: COLORS.gray, fontWeight: '600', marginBottom: '2px' }}>TOTAL</div>
              <div style={{ fontSize: '18px', fontWeight: '700', color: COLORS.dark }}>{total.toLocaleString()} {displayUnit}</div>
            </div>
            <div style={{ width: '1px', backgroundColor: COLORS.border }} />
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '10px', color: COLORS.gray, fontWeight: '600', marginBottom: '2px' }}>AVG/DAY</div>
              <div style={{ fontSize: '18px', fontWeight: '700', color: COLORS.primary }}>{Math.round(avg).toLocaleString()} {displayUnit}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  const topMetrics = useMemo(() => {
    ...
    return {
      topConsumed: topConsumed ? {
        ...
        category: topConsumed.category,
        value: topConsumed.totalConsumed.toLocaleString(),
        unit: topConsumed.item.unit || topConsumed.item.unitOfMeasurement || 'units',
        subtext: `${topConsumed.avgDaily.toFixed(1)} ${topConsumed.item.unit || topConsumed.item.unitOfMeasurement || 'units'} per day`
      } : null,
      fastMoving: fastMoving ? {
        itemName: fastMoving.item.itemName,
        category: fastMoving.category,
        value: fastMoving.avgDaily.toFixed(1),
        unit: `${fastMoving.item.unit || fastMoving.item.unitOfMeasurement || 'units'}/day`,
        subtext: `${fastMoving.totalConsumed.toLocaleString()} ${fastMoving.item.unit || fastMoving.item.unitOfMeasurement || 'units'} total consumed`
      } : null,
      slowMoving: slowMoving ? {
        itemName: slowMoving.item.itemName,
        category: slowMoving.category,
        value: slowMoving.avgDaily.toFixed(1),
        unit: `${slowMoving.item.unit || slowMoving.item.unitOfMeasurement || 'units'}/day`,
        subtext: `${slowMoving.totalConsumed.toLocaleString()} ${slowMoving.item.unit || slowMoving.item.unitOfMeasurement || 'units'} total consumed`
      } : null,
      lowCategory: lowCategory ? {
        itemName: lowCategory.name,
        category: 'Category-level',
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div>
              <div style={{ marginBottom: '16px' }}>
                <h2 style={{ fontSize: '18px', fontWeight: '700', color: COLORS.dark, margin: '0 0 2px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  AI-Powered Smart Insights
                  <span style={{ fontSize: '20px' }}>🤖</span>
                </h2>
                <p style={{ fontSize: '12px', color: COLORS.gray, margin: 0 }}>
                  Intelligent recommendations based on consumption patterns and predictive analytics
                </p>
              </div>
    ...
    <Card>
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '20px', marginBottom: 12 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.dark, margin: 0, flexShrink: 0 }}>
            Core Inventory
          </h3>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <th style={{ padding: '10px 8px', position: 'relative' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <button
                    ...
                    style={{
                      background: selectedCategoryFilter ? COLORS.primary : COLORS.bg,
                      border: 'none',
                      borderRadius: '4px',
                      padding: '4px 6px',
                      cursor: 'pointer',
                  ...
                  >
                    <Filter size={14} color={selectedCategoryFilter ? COLORS.white : COLORS.gray} />
                  </button>
                </div>
                      ...
                      {categories.map(cat => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{ fontSize: '11px', color: COLORS.gray }}>({itemCount})</span>
                          </div>
                        );
                      })}
                    </div>
          ...
          <tbody>
            {visibleItems.map((it) => {
              ...
              const coverage = avg > 0 ? Math.floor(current / avg) : Infinity;
              const categoryName = categories.find(c => c.id === it.categoryId)?.categoryName || 'Unknown';
              const unit = it.unit || it.unitOfMeasurement || 'units';
              ...
              return (
                <tr key={it.id} style={{ borderTop: `1px solid ${COLORS.border}` }}>
                  ...
                  <td style={{ padding: '12px 8px', verticalAlign: 'middle', textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 11, color: COLORS.gray }}>{unit}</div>
                  </td>
                  <td style={{ padding: '12px 8px', verticalAlign: 'middle', textAlign: 'right' }}>
                    <div style={{ fontWeight: 600, color: COLORS.dark }}>{avg}</div>
                    <div style={{ fontSize: 11, color: COLORS.gray }}>{unit}/day</div>
                  </td>
                  <td style={{ padding: '12px 8px', verticalAlign: 'middle', textAlign: 'right', color: coverage <= 7 ? COLORS.danger : COLORS.dark, fontWeight: 700 }}>
                    {coverage === Infinity ? '∞' : `${coverage}d`}
                  </td>
                </tr>
              );
