...
const DynamicCSVDashboard: React.FC = () => {
  ...
  // CSV file paths - memoized to prevent recreation
  ...
  // Process correlation data
  const processCorrelationData = useCallback((data: any[]): void => {
    const validData: CorrelationItem[] = data.filter(item => 
      item.item_name && 
      item.corr_top_positive_item && 
      item.item_name.toString().trim() !== '' &&
  ...
  // Process anomalies data
  const processAnomaliesData = useCallback((data: any[]): void => {
    setFullAnomaliesData(data);
  ...
  // Process stock data
  const processStockData = useCallback((data: any[]): void => {
    const processedData: StockItem[] = data.filter(item => item.item_name).map(item => ({
      ...item,
      consumption: Number(item.consumption) || 0,
      sih: Number(item.sih) || 0,
  ...
  // Generate consumption trends - wrapped in useCallback to prevent recreation
  const generateConsumptionTrendsFromAnomalies = useCallback((data: AnomalyItem[]): void => {
    if (!selectedItems.length || selectedItems.length !== 2 || !data.length) {
      setConsumptionTrends([]);
      return;
    }
  ...
  // Auto-load CSVs from paths on component mount - FIXED: removed csvPaths dependencies
  useEffect(() => {
    const autoLoadCSVs = async () => {
      console.log('🔄 Attempting to auto-load CSV files...');
      ...
      const anomaliesDataResult = await loadCSVFromPath(csvPaths.anomalies);
      if (anomaliesDataResult) {
        processAnomaliesData(anomaliesDataResult);
      } else {
        console.log('⚠️ Could not auto-load anomalies CSV, using file upload instead');
        setAnomaliesLoading(false);
      }

      const stockDataResult = await loadCSVFromPath(csvPaths.stock);
      if (stockDataResult) {
        processStockData(stockDataResult);
      } else {
        console.log('⚠️ Could not auto-load stock CSV, using file upload instead');
        setStockLoading(false);
      }
  ...
  // Update trends when selected items change - FIXED: removed function from dependencies
  useEffect(() => {
    if (!anomaliesLoading && fullAnomaliesData.length > 0) {
      generateConsumptionTrendsFromAnomalies(fullAnomaliesData);
    }
